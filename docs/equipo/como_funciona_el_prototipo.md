...
**Regla de oro de este documento:** todo lo que dice viene del código real del repositorio, revisado línea por línea el 2026-08-08 y actualizado de nuevo el 2026-08-09 (caso libre, Agente H, textos de interfaz corregidos, número de pruebas). Cuando algo no se ha podido confirmar del todo, aparece marcado como `[VERIFICAR]` en vez de dar por hecho algo que podría no ser cierto. Si en algún momento el código cambia, este documento puede quedarse desactualizado — conviene revisarlo antes de usarlo en la defensa si ha pasado tiempo.
...
- **El orden real es A → B → C → G → D → E.** Es decir, primero se comprueban los documentos (B) y se extraen los datos (C), y **solo después** se revisa el fraude (G). Un documento de una entrega anterior lo describía en otro orden; el código siempre ha ejecutado C antes que G porque los detectores de G necesitan los datos ya extraídos.
...
En el diseño original del proyecto (Entrega 1), el equipo definió una ficha completa para los ocho agentes, de la A a la H — F y H incluidos. El Agente H estaba pensado como un **copiloto de consulta para inspectores y abogados**: una herramienta apoyada en RAG para consultar el expediente una vez un caso llega a vía judicial. El Agente H **no se ha llegado a implementar** en ningún momento del proyecto — no existe como código, ni como parte de la aplicación, ni como módulo independiente como sí lo es el Agente F.

Su ausencia **no** se debe a que la letra se reservara desde el principio para más adelante — el tribunal tiene la Entrega 1, donde el Agente H ya tenía ficha propia. La causa real es cronológica: el alcance del MVP (los seis agentes A, B, C, G, D, E) se fijó en la Entrega 2, cerrada el 25 de junio de 2026 — semanas **antes** de que el cliente aportara, el 21 de julio, el dato de que solo se judicializan unos 10 expedientes al año. Es decir, el Agente H se dejó fuera del MVP por una **decisión de priorización de alcance del PoC** tomada ya en la Entrega 2, no por ese dato de julio; el dato de julio llegó después y sirvió para reforzar a posteriori una decisión que ya estaba tomada — y fue, además, el motivo de que se rediseñara el Agente F (ver sección 3). Por eso la numeración de los agentes implementados salta de la E a la G: no porque F y H no existieran en el diseño original, sino porque de los ocho agentes que sí tuvieron ficha completa en la Entrega 1, solo seis (A, B, C, G, D, E) más una versión rediseñada del F llegaron a implementarse en este prototipo.
